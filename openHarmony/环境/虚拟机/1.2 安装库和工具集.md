...
repo sync -c -j8        //这里一定要用j8 这样保证不会有文件下载不下来（-c 当前分支，-j8 用8个并行线程来执行repo sync）

repo forall -c 'git lfs pull'         //每个git仓库强制执行 拉取大文件命令

在源码根目录，执行prebuilts：
bash build/prebuilts_download.sh      //执行prebuilts进行预编译，安装编译器及二进制工具
