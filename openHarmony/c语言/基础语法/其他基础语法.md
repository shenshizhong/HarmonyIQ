...
```
```
ifndef 是 if not define 的缩写，是一种宏定义。
...
ifndef 和 endif 要一起使用。

```
#include 的作用
```
把include 后面那个文件内容，完完整整，包含到当前的文件夹中。

```
typedef 取一个别名
```
typedef struct Books  //给类取一个别名为Books
{
   char  title[50];
   char  author[50];
   char  subject[100];
...
作用：简化代码
```
[详细地址：](https://blog.csdn.net/xiaoquantouer/article/details/51647865)